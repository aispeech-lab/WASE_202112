...
if opt.restore:
    ...
    # model_dict = checkpoints['model']
    # only load voiceprint module parameters
    # model_dict = {k:v for k,v in model_dict.items() if 'encoder.rnn' in k or 'metric_fc.' in k or 'linear.' in k or 'criterion.' in k}
...
def train(epoch):
    ...
    while True:
        ...
        # writer.add_scalars('scalar/loss', {'voiceP_loss': voiceP_loss.cpu().item()}, updates)
        writer.add_scalars(
        ...
        if updates > 100 and updates % config.eval_interval in range(9, 10):
            ...
            # writer.add_scalars('scalar/SDRi', {'SDRi_train': SDRi_SUM.mean(),}, updates)
            # writer.add_scalars('scalar/SISNRi', {'SISNRi_train': SISNRi_SUM.mean(),}, updates)
            writer.add_scalars(
                'scalar/SDR', {'SDR_train': SDR_SUM.mean(), }, updates)
