...
class wase(nn.Module):
    ...
    def __init__(self, config, fre_size, frame_num, num_labels, use_cuda, score_fc):
        ...
        if self.config.BLOCKS == 'tcn':
            ...
            self.voiceprint_linear = nn.Linear(512, self.feature_dim)
    ...
    def forward(self, mix_wav, ref_wav, ref_wav_len, oracle_wav_endpoint):
    ...
    def get_voiceprint(self, ref_feas, ref_feas_len, pooling_methord='mean'):
        ...
        elif pooling_methord == 'max':
            voiceprint, _ = torch.max(voiceprint, dim=0)
        voiceprint = self.voiceprint_linear(voiceprint)
        return voiceprint
