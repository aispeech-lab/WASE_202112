...
def test(model, config, opt, writer, logging, updates=0, mode='valid'):
    ...
    save_samples_path = 'saved_samples'
    if os.path.exists(save_samples_path):
        print(" \ncleanup: " + save_samples_path + "/")
        shutil.rmtree(save_samples_path)
    ...
    while True:
        ...
        if eval_data is False:
            ...
            logging('accuracy_0_aver_eval_epoch: %f' % accuracy_0_SUM.mean())
            logging('f1_0_aver_eval_epoch: %f' % f1_0_SUM.mean())
            logging('accuracy_1_aver_eval_epoch: %f' % accuracy_1_SUM.mean())
            logging('f1_1_aver_eval_epoch: %f' % f1_1_SUM.mean())
            logging('accuracy_2_aver_eval_epoch: %f' % accuracy_2_SUM.mean())
            logging('f1_2_aver_eval_epoch: %f' % f1_2_SUM.mean())
            logging('accuracy_3_aver_eval_epoch: %f' % accuracy_3_SUM.mean())
            logging('f1_3_aver_eval_epoch: %f' % f1_3_SUM.mean())
            break
        ...
        utils.util.save_samples(
            config, predicted, eval_data, batch_idx, dst=save_samples_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-restore', default="onset_offset_voiceprint.pt", type=str,
    ...
    if opt.restore:
        ...
        checkpoints = torch.load(
            opt.restore, map_location={'cuda:2': 'cuda:0'})
    ...
    model = models.wase(config, fre_size, frame_num,
                        num_labels, use_cuda, opt.score_fc)
    if opt.restore:
