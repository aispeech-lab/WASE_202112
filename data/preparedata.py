...
def prepare_data(mode, train_or_test='train'):
    ...
    for batch_num in range(batch_total + 1):
        ...
        for batch_idx in range(batch_size):
            ...
            # mix_feas = get_stft_feas(mix_wav, config.FRAME_LENGTH, config.FRAME_SHIFT)
            # mix_feas_list.append(mix_feas)
            # mix_phases = get_stft_feas(mix_wav, config.FRAME_LENGTH, config.FRAME_SHIFT, feas_type="phase")
            # mix_phases_list.append(mix_phases)
            ...
            # aim_feas = get_stft_feas(aim_wav, config.FRAME_LENGTH, config.FRAME_SHIFT)
            # aim_feas_list.append(aim_feas)

            mask_wav_list.append(mask_wav)
            # mask_feas = get_stft_feas(mask_wav, config.FRAME_LENGTH, config.FRAME_SHIFT)
            ...
            # spk_feas_dict_this_sample[target_id] = aim_feas
            # spk_feas_dict_this_sample[mask_id] = mask_feas
            ...
            # spk_feas_dict_list.append(spk_feas_dict_this_sample)
        ...
        # energy_sorted_ss_aim_spk = get_energy_order(spk_feas_dict_list)
        # sorted_ss_aim_spk = [spk_feas.keys() for spk_feas in spk_feas_dict_list]
        ...
        if mode == 'global':
            ...
            yield {'all_spk': all_spk,
                   ...
                   # 'num_fre': mix_feas.shape[1],
                   # 'num_frames': mix_feas.shape[0],
                   'num_fre': 129,
                   'num_frames': 100,
                   'spk_num': len(all_spk),
            ...
            yield {'mix_wav': mix_wav_list,
                   # 'mix_feas': mix_feas_list,
                   # 'mix_phases': mix_phases_list,
                   'aim_wav': aim_wav_list,
                   # 'aim_feas': aim_feas_list,
                   'aim_spk': aim_spk,
                   ...
                   # 'spk_feas_dict_list': spk_feas_dict_list,
